...
void cMapManager::m_addDestructibles(void)
{
    // Map file
    cXML mapFile;
    ...
    {
        // Map Objects
        m_map->numDebris = mapFile.getInstanceCount("<destructible_object>");
        if (m_map->numDebris > 0)
        {
            ...
            {
                // Get data
                dataString  = mapFile.getString("<destructible_object>", i + 1);
