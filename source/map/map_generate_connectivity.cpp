...
void cMapManager::m_mapConnectRooms_SL(sMap*& _map, const std::int32_t &_r1, const std::int32_t &_r2)
{
    ...
    if (direction == eDirectionBias::directionEast)
    {
        while ((_map->tile[(psy * _map->info.size_x) + psx].type == eTileType::tileWallTall) && (psx < _map->room[_r1].posXMax))
        {
            psx++;
        }
        while ((_map->tile[(pey * _map->info.size_x) + pex].type == eTileType::tileWallTall) && (pex > _map->room[_r2].posXMin))
        {
            pex--;
        }
    }
    if (direction == eDirectionBias::directionWest)
    {
        while ((_map->tile[(psy * _map->info.size_x) + psx].type == eTileType::tileWallTall) && (psx > _map->room[_r1].posXMin))
        {
            psx--;
        }
        while ((_map->tile[(pey * _map->info.size_x) + pex].type == eTileType::tileWallTall) && (pex < _map->room[_r2].posXMax))
        {
            pex++;
        }
    }
    if (direction == eDirectionBias::directionNorth)
    {
        while ((_map->tile[(psy * _map->info.size_x) + psx].type == eTileType::tileWallTall) && (psy < _map->room[_r1].posYMax))
        {
            psy++;
        }
        while ((_map->tile[(pey * _map->info.size_x) + pex].type == eTileType::tileWallTall) && (pey > _map->room[_r2].posYMin))
        {
            pey--;
        }
    }
    if (direction == eDirectionBias::directionSouth)
    {
        while ((_map->tile[(psy * _map->info.size_x) + psx].type == eTileType::tileWallTall) && (psy > _map->room[_r1].posYMin))
        {
            psy--;
        }
        while ((_map->tile[(pey * _map->info.size_x) + pex].type == eTileType::tileWallTall) && (pey < _map->room[_r2].posYMax))
        {
            pey++;
        }
    }
    ...
    {
        std::uint32_t tile = (psy * _map->info.size_x) + psx;
        if (_map->tile[tile].type == eTileType::tileWallTall)
        {
            _map->tile[tile].type = eTileType::tileFloor;
            if (_map->generate->connectivityPadding > 0)
            {
                for (std::uint32_t i = 0; i <= _map->generate->connectivityPadding; i++)
                {
                    if (_map->tile[tile + i].type == eTileType::tileWallTall)
                        _map->tile[tile + i].type = eTileType::tileFloor;
                    if (_map->tile[tile - i].type == eTileType::tileWallTall)
                        _map->tile[tile - i].type = eTileType::tileFloor;
                    if (_map->tile[tile + _map->info.size_x].type == eTileType::tileWallTall)
                        _map->tile[tile + _map->info.size_x].type = eTileType::tileFloor;
                    if (_map->tile[tile - _map->info.size_x].type == eTileType::tileWallTall)
                        _map->tile[tile - _map->info.size_x].type = eTileType::tileFloor;
                    if (_map->tile[tile + _map->info.size_x + i].type == eTileType::tileWallTall)
                        _map->tile[tile + _map->info.size_x + i].type = eTileType::tileFloor;
                    if (_map->tile[tile + _map->info.size_x - i].type == eTileType::tileWallTall)
                        _map->tile[tile + _map->info.size_x - i].type = eTileType::tileFloor;
                    if (_map->tile[tile - _map->info.size_x + i].type == eTileType::tileWallTall)
                        _map->tile[tile - _map->info.size_x + i].type = eTileType::tileFloor;
...
void cMapManager::m_mapConnectRooms_ND(sMap*& _map, const std::int32_t &_r1, const std::int32_t &_r2)
{
    ...
    while (psy != pey)
    {
        ...
        if (_map->tile[tile].type == eTileType::tileWallTall)
        {
            _map->tile[(psy * _map->info.size_x) + psx].type = eTileType::tileFloor;
            if (_map->generate->connectivityPadding > 0)
            {
                for (std::uint32_t i = 0; i <= _map->generate->connectivityPadding; i++)
                {
...
void cMapManager::m_mapAddDooryways(sMap*& _map)
{
    for (std::uint32_t i = 0; i < _map->numRoom; i++)
    {
        ...
        // y-axiz
        for (std::uint32_t y = 0; y < _map->room[i].h; ++y)
        {
            tile = ((_map->room[i].y - (_map->room[i].h / 2) + y) * _map->info.size_x) + (_map->room[i].x - (_map->room[i].w / 2) - 1);
        ...
        // x-axiz
        for (std::uint32_t x = 0; x < _map->room[i].w; ++x)
        {
            tile = ((_map->room[i].y - (_map->room[i].h / 2) - 1) * _map->info.size_x) + (_map->room[i].x - (_map->room[i].w / 2) + x);
            if (_map->tile[tile].type == eTileType::tileFloor)
                _map->tile[tile].type = eTileType::tileDoorway;
            tile = ((_map->room[i].y + (_map->room[i].h / 2) + 1) * _map->info.size_x) + (_map->room[i].x - (_map->room[i].w / 2) + x);
            if (_map->tile[tile].type == eTileType::tileFloor)
...
void cMapManager::m_mapConnectRooms(sMap*& _map)
{
    ...
    for (std::uint32_t i = 0; i < _map->numRoom; i++)
    {
        ...
        {
            // right / west
            for (std::uint32_t k = _map->room[i].x; k < _map->info.size_x; k++)
            {
        ...
        {
            // down / south
            for (std::uint32_t j = _map->room[i].y; j < _map->info.size_y; j++)
            {
                std::uint32_t r = _map->tile[(j * _map->info.size_x) + k].room;
                if ((_map->tile[(j * _map->info.size_x) + k].type == eTileType::tileFloor) && (_map->tile[(j * _map->info.size_x) + k].room != i))
                {
                    ...
                    {
                        distanceS = distanceT;
                        _map->room[i].exitS = _map->tile[(j * _map->info.size_x) + k].room;
                        _map->room[_map->tile[(j * _map->info.size_x) + k].room].exitN = i;
                    }
                }
            }
            // up / north
            for (std::uint32_t j = _map->room[i].y; j > 0; j--)
            {
                ...
                if ((_map->tile[(j * _map->info.size_x) + k].type == eTileType::tileFloor) && (_map->tile[(j * _map->info.size_x) + k].room != i))
                {
                    ...
                    {
                        distanceN = distanceT;
                        _map->room[i].exitN = _map->tile[(j * _map->info.size_x) + k].room;
                        _map->room[_map->tile[(j * _map->info.size_x) + k].room].exitS = i;
                    }
                }
            }
        }
    ...
    for (std::uint32_t i = (_map->numRoom-1); i > 0; i--)
    {
    ...
    if (_map->numRoom > 0)
    {
        ...
        {
            sMapPath pathT;
            pathT.startTile       = ((_map->info.size_x * _map->room[i].y)   + _map->room[i].x);
            pathT.destinationTile = ((_map->info.size_x * _map->room[i+1].y) + _map->room[i+1].x);
            gAStar(_map, pathT, false, false);
            if (pathT.pathLength == 0)
            {
                if (_map->generate->connectivityAlgorithm == eConnectAlgo::algorithm_SL)
                    m_mapConnectRooms_SL(_map, i, i + 1);
                if (_map->generate->connectivityAlgorithm == eConnectAlgo::algorithm_ND)
                    m_mapConnectRooms_ND(_map, i, i + 1);
                if (_map->generate->connectivityAlgorithm == eConnectAlgo::algorithm_DW)
