...
void setup_skillbook_window(int UID)
{
    ...
    game.window_manager.window[window_number].texture.base            = game.texture_manager.add_texture("data/textures/UI/menu/skill_book_background.png");
    ...
    if(game.window_manager.window[window_number].event.id > EVENT_NONE)
    {
        switch (game.window_manager.window[window_number].event.id)
        {
            ...
            case (EVENT_ELEMENT_DROP): //Element drop event posted
                //game.window_manager.destination.window = SKILLBOOK_UID;
            break;
            default:
