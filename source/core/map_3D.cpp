...
void map_3D_class::draw(void)
{
    ...
    if (map_3D_class::render_textured)
    {
        ...
        for(int cell_count = 0; cell_count < map_3D_class::number_of_cells; cell_count++)
        {
            if (set_texture)
            {
                //glBindTexture( GL_TEXTURE_2D, game.resource.texture.generic_grass.frame[0].data);
            ...
            {
                //game.resource.model_3D.tree_fern.draw(map_3D_class::cell[0].x,map_3D_class::cell[0].y,map_3D_class::cell[0].z);
                set_texture = true;
            }
        }
    }
    ...
    if (map_3D_class::render_surfaces)
    {
        for(int cell_count = 0; cell_count < map_3D_class::number_of_cells; cell_count++)
        {
            if (map_3D_class::cell_visable(cell_count))
            {
                ...
                if (cell_count == 0) //(map_3D_class::cell[cell_count].object > 0)
                {
                    //game.resource.model_3D.tree_fern.draw    (map_3D_class::cell[0].vertex[0].x,map_3D_class::cell[0].vertex[0].y,map_3D_class::cell[0].z);
                    //game.resource.model_3D.tree_palm.draw    (map_3D_class::cell[2].vertex[0].x,map_3D_class::cell[2].vertex[0].y,map_3D_class::cell[2].z);
                    //game.resource.model_3D.well.draw         (map_3D_class::cell[103].vertex[0].x,map_3D_class::cell[103].vertex[0].y,map_3D_class::cell[103].z);
                    //game.resource.model_3D.tombwoof.draw     (map_3D_class::cell[205].vertex[0].x,map_3D_class::cell[205].vertex[0].y,map_3D_class::cell[205].z);
                    //game.resource.model_3D.tomb.draw         (map_3D_class::cell[207].vertex[0].x,map_3D_class::cell[207].vertex[0].y,map_3D_class::cell[207].z);
                    //game.resource.model_3D.cemetery_gate.draw(map_3D_class::cell[409].vertex[0].x,map_3D_class::cell[409].vertex[0].y,map_3D_class::cell[409].z);
                    set_texture = true;
                }
            }
        }
    ...
    {
        //glBindTexture( GL_TEXTURE_2D, game.resource.texture.generic_water.frame[0].data);
        glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_OBJECT_LINEAR);
        glTexGeni(GL_T, GL_TEXTURE_GEN_MODE, GL_OBJECT_LINEAR);
        glEnable(GL_TEXTURE_GEN_S);
        glEnable(GL_TEXTURE_GEN_T);
