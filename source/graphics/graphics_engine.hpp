...
class cGraphicsEngine
{
    public:
        ...
        void          setDisplay(const std::uint32_t &_w, const std::uint32_t &_h, const bool &_f) {m_window_w = _w; m_window_h = _h; m_fullscreen = _f; };
        ...
        float         getAspectRatio(void) { return m_aspectRatio; };
        void          addScreenShake(const float &_st, const float &_sf) { m_camera.addScreenShake(_st, _sf); }
        void          setParticleTextureID(const std::uint32_t &_tid) { m_p4_particleTextureID = _tid; }
