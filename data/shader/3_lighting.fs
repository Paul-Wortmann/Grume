...
in vec4 vs_billboardPosition;
flat in int  vs_billboard;
...
float ShadowCalculation(const int _lightNum);
float ShadowCalculation2(const int _lightNum);
vec3  PointLightCalculation(const int _lightNum, vec3 _normal);
...
void main()
{
    if (vs_billboard > 0)
    {
        if (isText > 0)
        {
            fragColor = vec4(1.0, 1.0, 1.0, texture(material.diffuse, vs_textcoord).r);
            fragColor *= textColor;
        }
        else
        {
            fragColor = texture(material.diffuse, vs_textcoord);
        }
    }
    else
    {
